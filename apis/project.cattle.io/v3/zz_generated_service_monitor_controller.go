...
func NewServiceMonitor(namespace, name string, obj ServiceMonitor) *ServiceMonitor {
...
type ServiceMonitorHandlerFunc func(key string, obj *ServiceMonitor) (runtime.Object, error)

type ServiceMonitorChangeHandlerFunc func(obj *ServiceMonitor) (runtime.Object, error)

type ServiceMonitorLister interface {
	List(namespace string, selector labels.Selector) (ret []*ServiceMonitor, err error)
	Get(namespace, name string) (*ServiceMonitor, error)
}
...
type ServiceMonitorInterface interface {
	...
	GetNamespaced(namespace, name string, opts metav1.GetOptions) (*ServiceMonitor, error)
...
func (l *serviceMonitorLister) List(namespace string, selector labels.Selector) (ret []*ServiceMonitor, err error) {
	err = cache.ListAllByNamespace(l.controller.Informer().GetIndexer(), namespace, selector, func(obj interface{}) {
		ret = append(ret, obj.(*ServiceMonitor))
	})
	return
}

func (l *serviceMonitorLister) Get(namespace, name string) (*ServiceMonitor, error) {
	var key string
	if namespace != "" {
		key = namespace + "/" + name
	} else {
...
func (c *serviceMonitorController) AddHandler(ctx context.Context, name string, handler ServiceMonitorHandlerFunc) {
	c.GenericController.AddHandler(ctx, name, func(key string, obj interface{}) (interface{}, error) {
		...
		} else if v, ok := obj.(*ServiceMonitor); ok {
...
func (c *serviceMonitorController) AddClusterScopedHandler(ctx context.Context, name, cluster string, handler ServiceMonitorHandlerFunc) {
	c.GenericController.AddHandler(ctx, name, func(key string, obj interface{}) (interface{}, error) {
		if obj == nil {
			return handler(key, nil)
		} else if v, ok := obj.(*ServiceMonitor); ok && controller.ObjectInCluster(cluster, obj) {
			return handler(key, v)
		} else {
			return nil, nil
		}
...
func (s *serviceMonitorClient) Create(o *ServiceMonitor) (*ServiceMonitor, error) {
	obj, err := s.objectClient.Create(o)
	return obj.(*ServiceMonitor), err
}

func (s *serviceMonitorClient) Get(name string, opts metav1.GetOptions) (*ServiceMonitor, error) {
	obj, err := s.objectClient.Get(name, opts)
	return obj.(*ServiceMonitor), err
}

func (s *serviceMonitorClient) GetNamespaced(namespace, name string, opts metav1.GetOptions) (*ServiceMonitor, error) {
	obj, err := s.objectClient.GetNamespaced(namespace, name, opts)
	return obj.(*ServiceMonitor), err
}

func (s *serviceMonitorClient) Update(o *ServiceMonitor) (*ServiceMonitor, error) {
...
func (s *serviceMonitorClient) Patch(o *ServiceMonitor, patchType types.PatchType, data []byte, subresources ...string) (*ServiceMonitor, error) {
