...
type PodSecurityPolicyController interface {
	...
	AddClusterScopedHandler(clusterName string, handler PodSecurityPolicyHandlerFunc)
...
type PodSecurityPolicyInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync PodSecurityPolicyHandlerFunc)
...
func (c *podSecurityPolicyController) AddClusterScopedHandler(cluster string, handler PodSecurityPolicyHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *podSecurityPolicyClient) AddClusterScopedSyncHandler(clusterName string, sync PodSecurityPolicyHandlerFunc) {
