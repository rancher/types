...
type ClusterController interface {
	...
	AddClusterScopedHandler(clusterName string, handler ClusterHandlerFunc)
...
type ClusterInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync ClusterHandlerFunc)
...
func (c *clusterController) AddClusterScopedHandler(cluster string, handler ClusterHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *clusterClient) AddClusterScopedSyncHandler(clusterName string, sync ClusterHandlerFunc) {
