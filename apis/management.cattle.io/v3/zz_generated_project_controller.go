...
type ProjectController interface {
	...
	AddClusterScopedHandler(clusterName string, handler ProjectHandlerFunc)
...
type ProjectInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync ProjectHandlerFunc)
...
func (c *projectController) AddClusterScopedHandler(cluster string, handler ProjectHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *projectClient) AddClusterScopedSyncHandler(clusterName string, sync ProjectHandlerFunc) {
