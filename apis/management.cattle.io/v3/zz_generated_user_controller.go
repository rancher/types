...
type UserController interface {
	...
	AddClusterScopedHandler(clusterName string, handler UserHandlerFunc)
...
type UserInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync UserHandlerFunc)
...
func (c *userController) AddClusterScopedHandler(cluster string, handler UserHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *userClient) AddClusterScopedSyncHandler(clusterName string, sync UserHandlerFunc) {
