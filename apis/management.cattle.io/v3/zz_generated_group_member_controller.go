...
type GroupMemberController interface {
	...
	AddClusterScopedHandler(clusterName string, handler GroupMemberHandlerFunc)
...
type GroupMemberInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync GroupMemberHandlerFunc)
...
func (c *groupMemberController) AddClusterScopedHandler(cluster string, handler GroupMemberHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *groupMemberClient) AddClusterScopedSyncHandler(clusterName string, sync GroupMemberHandlerFunc) {
