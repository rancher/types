...
type EventController interface {
	...
	AddClusterScopedHandler(clusterName string, handler EventHandlerFunc)
...
type EventInterface interface {
	...
	AddClusterScopedSyncHandler(clusterName string, sync EventHandlerFunc)
...
func (c *eventController) AddClusterScopedHandler(cluster string, handler EventHandlerFunc) {
	c.GenericController.AddHandler(func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *eventClient) AddClusterScopedSyncHandler(clusterName string, sync EventHandlerFunc) {
